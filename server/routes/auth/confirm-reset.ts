...
<html>
  ...
  <body>
    <main>
      <h1>Reset PIN</h1>
      <p>Set a new 4-digit Micham PIN. After saving, return to the app and login again.</p>
      ${message ? `<p class="msg">${message}</p>` : ""}
      <p id="form-message" class="msg"></p>
      <label>New PIN<input id="password" type="password" inputmode="numeric" maxlength="4" pattern="[0-9]{4}" autocomplete="new-password" /></label>
      <label>Confirm PIN<input id="confirm" type="password" inputmode="numeric" maxlength="4" pattern="[0-9]{4}" autocomplete="new-password" /></label>
      ...
      <script>
        document.getElementById("save").addEventListener("click", async () => {
          ...
          const response = await fetch("/api/auth/confirm-reset", {
            ...
            body: JSON.stringify({ token: ${JSON.stringify(token)}, pin: password })
          });
          const result = await response.json().catch(() => ({}));
          document.querySelector("main").innerHTML = response.ok
            ? "<h1>PIN changed</h1><p>Your PIN has been updated. Return to Micham and login.</p>"
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  try {
    ...
    const password = stringField(body, "pin") || stringField(body, "password");
    if (!token) throw new ApiError(400, "Reset token is required.");
    if (!/^\d{4}$/.test(password)) throw new ApiError(400, "PIN must be exactly 4 digits.");
