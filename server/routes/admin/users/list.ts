...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  try {
    ...
    const limits = userIds.length
      ? await adminDb().from("micham_user_storage_limits").select("user_id, limit_bytes").in("user_id", userIds)
      : { data: [], error: null };
    if (limits.error) throw limits.error;
    const limitByUser = new Map((limits.data || []).map((row) => [row.user_id, row.limit_bytes]));
    jsonOk(res, {
      users: (data || []).map((user) => ({ ...user, receipt_storage_limit_bytes: limitByUser.get(user.id) ?? null })),
      page,
      pageSize,
      total: count || 0,
    });
