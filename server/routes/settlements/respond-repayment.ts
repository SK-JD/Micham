...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  try {
    ...
    const recipientId = event.requested_by || (event.owner_id === user.id ? event.friend_id : event.owner_id);
    if (recipientId) {
      void sendPushToUser(recipientId, {
        title: action === "accept" ? "Settlement approved" : "Settlement rejected",
        body: `${user.display_name} ${action === "accept" ? "approved" : "rejected"} your returned-money update.`,
        tag: `settlement-${action}`,
        url: "/",
      });
    }
