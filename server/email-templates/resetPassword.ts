...
export function resetPasswordTemplate(displayName: string, resetUrl: string) {
  const text = `Hi ${displayName}, reset your Micham PIN: ${resetUrl}`;
  const html = mailShell(
    "Reset your Micham PIN",
    "Use this secure link to set a new 4-digit PIN.",
    `
      <p style="margin:0 0 16px;color:#315f51;font-size:15px;line-height:1.6">Hi ${escapeHtml(displayName)},</p>
      <p style="margin:0 0 24px;color:#315f51;font-size:15px;line-height:1.6">Use this secure link to set a new 4-digit PIN. The link expires soon.</p>
      <p style="margin:0 0 24px">${buttonLink(resetUrl, "Reset PIN")}</p>
  ...
  return { subject: "Reset your Micham PIN", html, text };
}
